...
def read_training_data(training_directory):
    ...
    # path, dirs, files = next(os.walk(training_directory))
    # file_count = len(files)
    # print("file_count",file_count)

    image_data = []
    target_data = []
    print("training_directory",training_directory)

    for each_letter in letters:

        image_path = os.path.join(training_directory, each_letter)
        # path, dirs, files = next(os.walk(training_directory,str(each_letter)))
        print("image_path",image_path)
        file_count = len(image_path)-3
        # print("files",files)
        print("file_count",file_count)
